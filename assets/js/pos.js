...
if (auth == undefined) {
  ...
  $(document).ready(function () {
    ...
    $.fn.calculateCart = function () {
      ...
      let grossTotal;
      let total_items = 0;
      totalVat = 0; // global totalVat
      $.each(cart, function (index, data) {
        let item_total = data.quantity * data.price;
        total += item_total;
        total_items += parseInt(data.quantity);
        if (settings.charge_tax) {
          let itemGst = parseFloat(data.gst || 0);
          totalVat += (item_total * itemGst) / 100;
        }
      });
    ...
    $.fn.submitDueOrder = function (status) {
      ...
      if (requiresPrescription && status === 1) {
        ...
        if (!patient || !prescriber) {
          notiflix.Report.warning(
            ...
            "Please enter Patient Name and Doctor/Prescriber Name for Schedule H/H1 drugs.",
            "Ok"
          );
          return;
        }
      ...
      if (requiresPrescription) {
        data.prescriptionRecord = {
          patientName: $("#prescriptionPatient").val(),
          prescriberName: $("#prescriptionPrescriber").val(),
          prescriberRegNo: $("#prescriptionRegNo").val(),
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $("#prescriptionPatient").val("");
          $("#prescriptionPrescriber").val("");
          $("#prescriptionRegNo").val("");
    ...
    $.fn.editProduct = function (index) {
      ...
      $("#gst").val(allProducts[index].gst || "0");
      $("#hsnCode").val(allProducts[index].hsnCode || "");
      $("#scheduleType").val(allProducts[index].scheduleType || "OTC");
